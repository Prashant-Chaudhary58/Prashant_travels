...
import soloIcon from "../assets/addprop/solo.png";
import coupleIcon from "../assets/addprop/couple.png";
import familyIcon from "../assets/addprop/family.png";
...
const AddPackageForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append("package_type", activeType);
      ...
      const response = await fetch(
        "http://localhost:5000/package/registerPackage",
        {
          method: "POST",
          headers: {
      ...
      console.log(responseData,"______________resp");
      

      if (!response.ok) {
        throw new Error(responseData.error || "Failed to upload package");
      }
      ...
      toast.error(error.message || "Failed to upload package");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      ...
      <div className="container px-8 py-8 max-w-5xl mx-auto mt-35">
        <h2 className="text-3xl font-bold mb-4">List your Package .</h2>
        <p className="text-lg text-gray-700 mb-8">Select your Package Type .</p>

        <div className="package-type flex justify-between gap-4 mb-8">
          {[
            { type: "family", icon: familyIcon, label: "Family Package" },
            { type: "solo", icon: soloIcon, label: "Solo Package" },
            { type: "couple", icon: coupleIcon, label: "Couple Package" },
            { type: "Premium", icon: hotelIcon, label: "Premium" },
          ].map(({ type, icon, label }) => (
            <div
              key={type}
              className={`type-card w-64 flex flex-col items-center py-8 px-6 border-2 rounded-lg text-center cursor-pointer transition-colors duration-300 ${
                activeType === type
                  ? "border-indigo-500"
        ...
        <div className="form-container flex gap-8 items-start">
          ...
          <form
            ...
            className="package-form flex-1 flex flex-col gap-3"
          >
            <input
              name="title"
              type="text"
            ...
            <div className="offerings flex flex-wrap gap-2">
              {Object.keys(offeringIcons).map((offerName) => (
                <button
                  key={offerName}
                  type="button"
                  className={`offer flex items-center gap-2 bg-gray-200 text-gray-700 py-2 px-4 rounded-md text-sm transition-colors duration-300 ${
            ...
            <button
              type="submit"
              className="upload-btn bg-indigo-600 text-white hover:bg-indigo-500 py-3 px-6 rounded-md text-lg"
              disabled={loading}
            >
